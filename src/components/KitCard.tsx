  ...
  return (
    <div className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md hover:border-blue-200 transition-all duration-200">
      <div className="relative h-44 bg-gray-50 overflow-hidden cursor-pointer" onClick={() => onView(kit)}>
        {imageUrl ? <img src={imageUrl} alt={kit.name} className="w-full h-full object-cover object-left-top group-hover:scale-[1.02] transition-transform duration-300"/> : <div className="w-full h-full flex items-center justify-center text-gray-300"><svg width="48" height="48" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg></div>}
      ...
      <div className="p-4">
        ...
        {kit.description ? <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed mb-3">{kit.description}</p> : <p className="text-xs text-gray-300 italic mb-3">Sem descrição</p>}
