  ...
  function handleJsonChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => {
      try {
        ...
        setError(null)
      } catch (err: any) { setError('JSON invÃÂ¡lido: ' + err.message) }
    }
    reader.readAsText(file)
  }

  async function handleSubmit() {
    if (!name.trim()) { setError('Nome ÃÂ© obrigatÃÂ³rio.'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div><label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">DescriÃÂ§ÃÂ£o</label><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="InstalaÃÂ§ÃÂ£o tÃÂ­pica para..." rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400 resize-none"/></div>
          <div><label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Imagem do Diagrama</label><div className="border-2 border-dashed border-gray-200 rounded-xl overflow-hidden cursor-pointer hover:border-blue-300 transition-colors" onClick={() => imgInputRef.current?.click()}>{imagePreview ? <img src={imagePreview} alt="preview" className="w-full h-40 object-contain"/> : <div className="h-32 flex flex-col items-center justify-center gap-2 text-gray-400"><Upload size={20}/><span className="text-xs">Clique para enviar imagem</span></div>}</div><input ref={imgInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageChange}/></div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading} className="flex-1 h-10 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 transition-colors">{loading ? 'Salvando...' : isEdit ? 'Salvar alteraÃÂ§ÃÂµes' : 'Criar kit'}</button>
        </div>
      </div>
    </div>
  )
