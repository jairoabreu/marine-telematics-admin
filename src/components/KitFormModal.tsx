  ...
  function handleJsonChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = ev => {
      try {
        ...
        setError(null)
      } catch (err: any) { setError('JSON inválido: ' + err.message) }
    }
    ...
    <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Instalação típica para..." rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400 resize-none"/>
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-xl overflow-hidden cursor-pointer hover:border-blue-300 transition-colors bg-gray-50" onClick={() => imgInputRef.current?.click()}>
                {imagePreview
                  ...
                  : <div className="h-32 flex flex-col items-center justify-center gap-2 text-gray-400"><Upload size={20}/><span className="text-xs">Clique para enviar imagem</span></div>
                }
              </div>
            ...
            <div>
              ...
              <button onClick={() => jsonInputRef.current?.click()} className="w-full h-9 border border-gray-200 rounded-lg px-3 text-xs text-gray-500 hover:bg-gray-50 hover:border-gray-300 transition-colors flex items-center gap-2">
                ...
                {jsonFileName
                  ? <span className="text-green-600 font-medium flex items-center gap-1.5"><CheckCircle2 size={12}/>{jsonFileName}</span>
                  : <span>Ou importar JSON existente</span>
                }
              </button>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
            ...
            <button onClick={handleSubmit} disabled={loading} className="flex-1 h-10 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 transition-colors">
              {loading ? 'Salvando...' : isEdit ? 'Salvar alterações' : 'Criar kit'}
            </button>
          </div>
        </div>
      </div>
      ...
      {showEditor && imagePreview && (
        <HotspotEditorModal
          ...
          onSave={(newHotspots) => { setHotspots(newHotspots); setJsonFileName(`${newHotspots.length} componentes desenhados`); setShowEditor(false) }}
        />
      )}
    </>
  )
